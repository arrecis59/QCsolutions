...
namespace CapaDatosMantenimientoAplicaciones
{
    ...
    {
        public void InsertarDatosAplicaciones(string codigoaplicacion, string nombreaplicacion)
        {
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=colchoneria"))
                {
...

                    {
                        ...
                        {
                            cmd.CommandText = "INSERT INTO tbl_aplicacion(PK_Api_codigo, api_descripcion, api_estado) VALUES(" + codigoaplicacion + ",'" + nombreaplicacion + "',1)";
                            cmd.ExecuteNonQuery();
        ...
        public void ModificarDatosModulo(string codigoaplicacionactual, string codigoaplicacion, string nombreaplicacion)
        {
            CadenaDeConexion cdc = new CadenaDeConexion();
            try
            {
                using (var conn = new OdbcConnection("dsn=colchoneria"))
                {
...

                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.CommandText = "UPDATE tbl_aplicacion SET PK_Api_codigo = " + codigoaplicacion + ", api_descripcion = '" + nombreaplicacion + "' WHERE PK_Api_codigo = '" + codigoaplicacionactual + "';";
                            cmd.ExecuteNonQuery();
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=colchoneria"))
                {
...

                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.CommandText = "SELECT tbl_aplicacion.PK_Api_codigo as Codigo, tbl_aplicacion.api_descripcion as Aplicacion FROM tbl_aplicacion WHERE api_estado = 1";
            ...
            try
            {
                using (var conn = new OdbcConnection("dsn=colchoneria"))
                {
                    ...
                    conn.Open();
                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.CommandText = "SELECT tbl_aplicacion.PK_Api_codigo, tbl_aplicacion.api_descripcion FROM tbl_aplicacion WHERE tbl_aplicacion.PK_Api_codigo = " + codigoapi + " AND api_estado = 1";
                            Reader = cmd.ExecuteReader();
                            while (Reader.Read())
                            {
